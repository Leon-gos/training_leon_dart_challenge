...
class StackAlignScreen extends StatelessWidget {
      ...
      return Scaffold(
        ...
        body: Container(
          ...
          child: Column(
            ...
            children: [
              _buildStackDemo(context, state),
              _buildAlignmentRow(context, state),
              _buildTextDirectionRow(context, state),
              _buildStackFitRow(context, state),
              _buildClipRow(context, state),
            ],
          ),
        ),
      );
  ...
  Widget _buildStackDemo(BuildContext context, StackState state) {
    return SizedBox(
      width: 200,
      height: 300,
      child: Stack(
  ...
  Widget _buildAlignmentRow(BuildContext context, StackState state) {
    return Row(
      children: [
  ...
  Widget _buildTextDirectionRow(BuildContext context, StackState state) {
    return Row(
      children: [
  ...
  Widget _buildStackFitRow(BuildContext context, StackState state) {
    return Row(
      children: [
        const Text("StackFit"),
        const Spacer(),
        DropdownButton(
          onChanged: (value) {
            context.read<StackBloc>().add(StackFitChange(stackFit: value!));
          },
          value: state.stackFit,
          items: const [
            DropdownMenuItem(value: StackFit.expand, child: Text("expand")),
            DropdownMenuItem(value: StackFit.loose, child: Text("loose")),
            DropdownMenuItem(
                value: StackFit.passthrough, child: Text("passthrough")),
          ],
        )
      ],
    );
  ...
  Widget _buildClipRow(BuildContext context, StackState state) {
    return Row(
      children: [
        const Text("Clip"),
        const Spacer(),
        DropdownButton(
          onChanged: (value) {
            context.read<StackBloc>().add(StackClipChange(clip: value!));
          },
          value: state.clip,
          items: const [
            DropdownMenuItem(value: Clip.none, child: Text("none")),
            DropdownMenuItem(value: Clip.hardEdge, child: Text("hardEdge")),
            DropdownMenuItem(value: Clip.antiAlias, child: Text("antiAlias")),
            DropdownMenuItem(
